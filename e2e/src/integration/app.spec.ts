...
/*
  it('it test the titles and messages of fetched heroes appears', ()=>{
    cy.visit('/')
    cy.contains('TOUR OF HEROES', {matchCase:false})
    messageFetchHeroes()
    cy.visit('/heroes')
...
*/
/*
  it('it test hero id is contain in the detail url and messega of fetch appear', ()=>{
    cy.visit('/detail/11')
    cy.url().then(url=>{
...
/*
  it('it test the a hero can be deleted and the number o elements is decrease by one', ()=>{
    cy.visit('/heroes')
    cy.get('[data-cy=list-of-heroes]').its('length').then(length=>{
